import { clamp, INNER_FEATURES_THRESHOLD, MAX_DEG, MIN_SWEEP, toDegrees, toRadians } from "./math.js";
import { shareOf } from "./slices.js";
 ...
 */
export function ratioOfPx(px, whole) {
 ...
 */
export function degreesOfArc(arcPx, radius) {
 ...
 */
export function computeGapDeg(sliceCount, visualGapDeg, ensureRenderableSlices) {
    if (sliceCount <= 1)
        return 0;
    const gap = Math.max(visualGapDeg, 0);
    if (!ensureRenderableSlices)
        return gap;
    const effectiveMinSweep = gap + MIN_SWEEP;
    if (sliceCount * effectiveMinSweep <= MAX_DEG)
        return gap;
 ...
 */
export function computeInnerGapDeg(gapDeg, holeRadiusRatio, innerRadius, hasSlices) {
    if (holeRadiusRatio < INNER_FEATURES_THRESHOLD)
        return 0;
    if (innerRadius <= 0 || gapDeg <= 0 || !hasSlices)
        return 0;
    return gapDeg;
}
/**
 ...
 */
export function computeTargetSweeps(data, total, gapDeg, ensureRenderableSlices, bandCount = 0, bandSweep = 0) {
    const n = data.length;
    const band = bandCount >= 1 && bandCount < n ? bandCount : 0;
    const sweeps = new Array(n).fill(0);
    const fill = (from, to, whole, degrees) => {
        for (let i = from; i < to; i++)
            sweeps[i] = shareOf(data[i].value, whole) * degrees;
    };
    if (band === 0) {
        ...
        for (let i = 1; i < band; i++)
            bandTotal = bandTotal.add(data[i].value);
 ...
 */
export function enforceMinSweepAngle(sweeps, minAngle) {
    let offset = 0;
    let diff = 0;
    ...
    if (diff > 0 && offset > 0) {
        for (let i = 0; i < sweeps.length; i++) {
            ...
            if (sweep > minAngle)
                sweeps[i] = sweep - ((sweep - minAngle) / diff) * offset;
        }
    }
}
/**
 ...
 */
export function computeCornerRadii(outerSweep, innerSweep, outerRadius, innerRadius, holeRadiusRatio, cornerRadiusRatio, roundInnerCorners, renderedSliceCount) {
    if (cornerRadiusRatio <= 0 || outerRadius <= 0)
    ...
    let cIn = holeRadiusRatio > INNER_FEATURES_THRESHOLD && roundInnerCorners && innerRadius > 0 && innerSweep > 0
        ? desiredC
        : 0;
    if (cIn > 0) {
...
export function sliceIndexAt(input) {
    ...
    for (let i = 0; i < input.segStarts.length; i++) {
        ...
        if (!isBand && input.fullSweeps[i] - gapDeg <= 0)
            continue; // not drawn, so not there to be tapped
