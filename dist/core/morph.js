 ...
 */
export const sliceKey = (slice) => slice.id;
/**
...
export class Morph {
    ...
    fromGap;
    toGap;
    constructor(renderList, renderIndexMap, from, to, bandFrom, bandTo, fractionSeed, fromGap, toGap) {
...
const sameNumbers = (a, b) => a.length === b.length && a.every((value, i) => value === b[i]);
/**
 ...
 */
export function planMorph(oldRender, oldMap, oldSweeps, newData, targetSweeps, fractionSeed, fromGap, toGap, oldBand, newBandCount) {
    // 1. Match old render items to new items by key. The first old item with a key wins.
    const matchOldToNew = new Array(oldRender.length).fill(-1);
    const matchNewToOld = new Array(newData.length).fill(-1);
    for (let i = 0; i < oldRender.length; i++) {
        if (oldMap[i] < 0)
            continue; // already exiting: stays exiting
    ...
    const exitBefore = new Array(oldRender.length).fill(0);
    let nextMatch = newData.length;
    for (let i = oldRender.length - 1; i >= 0; i--) {
        exitBefore[i] = nextMatch;
        if (matchOldToNew[i] >= 0)
            nextMatch = matchOldToNew[i];
    }
    ...
    for (let i = 0; i < oldRender.length; i++) {
        if (matchOldToNew[i] < 0 && exitBefore[i] === newData.length)
            addExiting(i);
    }
