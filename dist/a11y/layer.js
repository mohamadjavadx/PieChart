import { accessibleItems, actionForKey, DEFAULT_ACCESSIBLE_TEXT, initialActive, } from "./items.js";
const HIDDEN = {
...
export class AccessibilityLayer {
    ...
    seen = null;
    constructor(model, idPrefix, text = {}) {
        this.model = model;
    ...
    sync() {
        ...
        if (seen && seen.displayed === model.displayed && seen.band === model.band && seen.selected === model.selectedIndex &&
    ...
    showActive() {
        ...
        if (option)
            this.widget.setAttribute("aria-activedescendant", option.id);
        else
    ...
    activateItem(item) {
        if (item.kind === "group")
            ...
            this.model.activate(item.index);
    }
    onKeyDown(e) {
        if (e.altKey || e.ctrlKey || e.metaKey)
            return; // Browser and system shortcuts are theirs
        ...
        switch (action.type) {
            case "move": {
                ...
                if (item.kind === "slice" && !item.selected)
                    this.model.activate(item.index);
                this.showActive();
                break;
            }
