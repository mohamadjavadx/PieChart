...
import type { Slice, SliceInput } from "./types.ts";

/** The slices of [input] that have a value above zero, with exact values: what a chart shows for its data. */
export function toSlices(input: readonly SliceInput[]): Slice[] {
  const slices: Slice[] = [];
  for (const item of input) {
    const value = Decimal.from(item.value);
    if (value.signum() > 0) slices.push({ id: item.id, label: item.label, value, color: item.color });
  }
  return slices;
}

/** The sum of the values of [slices]: the whole that every share is a share of. */
export function totalOf(slices: readonly Slice[]): Decimal {
...
export function shareOf(value: Decimal, total: Decimal): number {
