...
export class CenterPresenter {
  ...
  displayed: SelectedSlice | null = null;
  /** 0..1, applied to everything the renderer draws. */
  alpha = 0;

  private wanted: SelectedSlice | null = null;
  private isAvailable = false;
  private fade: Fade | null = null;
  private readonly clock: () => number;
  private readonly onChange: () => void;

  constructor(clock: () => number, onChange: () => void) {
    this.clock = clock;
  ...
  update(slice: SelectedSlice | null, isAvailable: boolean): void {
    this.wanted = slice;
  ...
  private fadeTo(target: number, now: number): void {
    this.fade = null;
