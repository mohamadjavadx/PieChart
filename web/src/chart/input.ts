import type { DecimalInput } from "../core/decimal.ts";
import type { SliceId, SliceInput } from "../core/types.ts";
...
/** A slice as a page may give it: with its id and color left out. */
export interface LooseSliceInput {
  /** Defaults to the label (made unique when two slices share it), so a slice keeps its id across data changes. */
  readonly id?: SliceId;
  readonly label: string;
  readonly value: DecimalInput;
  /** Defaults to the next color of [DEFAULT_PALETTE]. */
  readonly color?: string;
}

/** Fills in the ids and colors that were left out. */
export function normalizeData(data: readonly LooseSliceInput[]): SliceInput[] {
  const taken = new Set<unknown>(data.filter((slice) => slice.id !== undefined).map((slice) => slice.id));
  return data.map((slice, i) => {
    let id = slice.id;
    if (id === undefined) {
      id = slice.label;
      for (let n = 2; taken.has(id); n++) id = `${slice.label} #${n}`;
      taken.add(id);
    }
...
export function sameData(a: readonly LooseSliceInput[], b: readonly LooseSliceInput[]): boolean {
